...
Pairs with **`pmo`**. Hand-off rule: **OKR proposes weekly tasks tagged with KR ids. PMO appends them to `tasks/<current-YYYY-MM>.md` after user approval.** OKR is the only writer of `OKR.md` and `monthly/`. PMO is the only writer of `tasks/`, `PROJECT_STATE.md`, `DECISIONS.md`, `evidence/`, `weekly/`, `handoff/`.
...
3. **Read** `tasks/<current-YYYY-MM>.md` (if PMO is also installed) so KR progress can be cross-checked against closed tasks and their evidence files.
...
9. **Not Doing** — explicit anti-goals scoped to this month. Often more concrete than the overall Anti-Goals. Examples: "no production deploys launch", "no multi-broker expansion", "no new paid API integrations".
...
#### `score [<YYYY-MM>]`
End-of-month retrospective. Cross-reference `evidence/<YYYY-MM>/` and `tasks/<YYYY-MM>.md` Done section.
...
5. On approval, **hand off to PMO**: print the exact task block list. PMO `add-task` writes them. OKR never writes `tasks/` directly.
...
- Open tasks grouped by KR (read from `tasks/<current-YYYY-MM>.md`)
...
| `tasks/<YYYY-MM>.md` | pmo | Read by OKR for cross-check; never written | (in pmo skill) |
...
- **Cite evidence paths.** Every progress claim points to a `tasks/<YYYY-MM>.md` row or `evidence/<YYYY-MM>/<file>.md`.
