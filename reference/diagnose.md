...
See `diagnose-explanations.md § The second rule: explain the mechanism, every time`.

### What the scan reports about work mode

See `diagnose-explanations.md § What the scan reports about work mode`.

### Plain-language glossary

See `diagnose-explanations.md § Plain-language glossary`.

### Where explanation goes in each stage

See `diagnose-explanations.md § Where explanation goes in each stage`.

### The other half of this rule

See `diagnose-explanations.md § The other half of this rule`.
...
| `MODE-02` | warn | **Retired with ADR-019; no longer emitted.** It reported `.perry/config.md § Tracks` disagreeing with `.perry/config.jsonl`; that file is deleted, so there is nothing left to disagree. The row stays so an old diagnosis that cites it still resolves. | — |
