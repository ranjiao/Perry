...
LINKAGE_STORE = "linkage.jsonl"


def load_linkage_store(state_root: Path) -> list[dict] | None:
    """Every record of `<state root>/linkage.jsonl`, or `None` when there is none.

    The same three-way answer `load_task_store` gives, for the same reason.
...
def linkage_from_store(records: list[dict]) -> Linkage:
    ...
    **Where each field comes from now**, all of it `schema/state-schema.json §
    ...
    for e in edges:
        tasks_by_kr.setdefault(str(e.get("kr") or ""), []).append(
            str(e.get("task") or ""))
    ...
    for oid in order:
        ...
        link.objectives.append(LinkageObjective(
            id=oid, title=titles.get(oid, ""), krs=by_objective[oid]))
    return link
...
#: The two spec sections that say what a round will write. **Structure only —
#: nothing here reads what is written under them.**
#:
#: The dispatch gate used to match the project's high-stakes fragments against
#: the text of these sections and return a verdict. That was Python judging a
#: document's meaning, which `ADR-007` decision 3 forbids, and five rounds of
#: correcting the match ended with the measurement that retired it: formatting
#: alone moved the verdict in both directions. `USER-916` removed it on
#: 2026-09-04; `work/reference/dispatch.md` pre-flight step 4 is now the
#: procedure a dispatching agent performs. TASK-339.
#:
#: What survives is the one question that is not about meaning: **does this
#: document offer either section at all.** A spec that offers neither gives a
#: reader nothing to judge from, and `bin/perry-lint` reports that across the
#: corpus before any single dispatch. Locating a heading is not parsing prose —
#: it is ADR-007 § 5b's "locate the file and hand it to an agent", and the
#: hand-off is what step 4 does with the answer.
#:
#: **These are canonical names, resolved through `alias()` — never matched
#: literally.** They read literally for eleven days anyway, because the glossary
#: carried no `zh` spelling for any of them while the hook heading on the other
#: side of the same gate (`High-stakes operations` → `高风险操作`) had one. Half
#: a gate internationalised is worse than none of it: a spec written in the
#: project's own declared document language found no sections and reported
#: nothing — TASK-200 measured three real specs through it. The fix was glossary
#: entries, not a second table here. TASK-201.
SPEC_SCOPE_SECTIONS = ("Files in scope", "Deliverable")


def spec_scope_sections(text: str) -> list[str]:
    """Which of `SPEC_SCOPE_SECTIONS` this spec offers, present and non-empty.

    `[]` means neither, so there is nothing in this document for a dispatching
    agent to screen. **That is not the same as clean**, and the whole reason
    this is reported: 45 of Perry's own 149 specs are in that state and every
    one of them used to scan `pass` over a fully armed 35-fragment union,
    byte-identically to a spec that had been read in full and found harmless.
    The gate's failure mode looked exactly like its success. TASK-284.

    A heading with nothing under it is a heading, not scope: `_section` returns
    the empty string for both, and counting the heading alone would report a
    reading that did not happen.

    Deliberately returns no verdict and matches no fragment. The judgement this
    feeds is `work/reference/dispatch.md` step 4.2, and it belongs to the agent.
    """
    body = _strip_comments(text or "")
    return [label for label in SPEC_SCOPE_SECTIONS
            if _section(body, *alias("headings", label)).strip()]
