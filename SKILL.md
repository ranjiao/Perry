...
Each lane's files are the ownership table below; each lane's SKILL.md carries its own subcommand index. Handled here instead of in a lane: the snapshot, `adopt`, `diagnose`, `relocate`, `help`, and writing `.perry/config.jsonl` at first-time setup.
...
1. **Read `.perry/config.jsonl`** (`"$PERRY_HOME/bin/perry-config" show --json`) for document language, chat language and repo layout. If it does not exist and a state file does, prompt for first-time setup. **Everything rendered from here uses the chat language**; files use `Document language`. Contract: `reference/i18n.md`.
...
2. **Run the namespace check before asking anything**, silently:
   ...
   - **`collisions: 0`** → the state root is `perry`, written in step 3, and **ask nothing** — the
...
3. **Confirm the project-wide preferences, then write them into `.perry/config.jsonl` before any other file.** The store is the first write of every start: a project whose only files are markdown is not installed (`schema/README.md § installed`) and would be sent back here on every session. Write it with the tool, never by hand:
   ...
   One `AskUserQuestion` call collects the answers: two questions, or three when step 2 found a collision:
   ...
   **Don't ask about chat language.** Set it to `follow user` (the second command above) and mirror what the user types. Document language governs **files**, chat language **replies**. Wordings: `reference/first-run.md`.
...
Settings live in `.perry/config.jsonl`, written with `perry-config set`; prose belongs in `.perry/hook.md`. First-time setup writes the store first, then the hook. Field **names** stay English in every language, because this file declares the language and must be readable before it is known. An optional `## Tracks` table turns on `pipeline` / `queue` / `inquiry` mode; absent means one implicit `main` track, mode `project`.
