...
- 2026-09-18 — § 5.7's "`OKR.md` renders the folded KR" rested on a premise that no longer holds: `OKR.md` has carried no KR rows since `TASK-236`, and KRs live only in `okr.jsonl`. So the folded overall KR and its `withdrawn <date>: <reason>` marker render in `perry-goals krs --level overall`, and every overall `kr` write leaves `OKR.md` byte-identical under the unchanged gate. An `OKR.md` that still carries KR rows refuses overall `kr` writes. The shipped `OKR_TEMPLATE.md` and `goals/reference/setup.md` dropped their KR tables in the same change (`USER-966`). Found by `TASK-264` D3, confirmed by its independent review. This corrects the premise; the design is unchanged.
