...
- 2026-09-16 — **phase A's gate becomes ≤ 75% of module-seconds, and phase A passes at 73.3%** — user decision `USER-943`, on `TASK-448` round 3's measurement. Eight modules came off the live state root (`tests/live_stores.py`), taking the evidence-only floor from 33.7% to **23.0%**, the broad declarations from 33 to 18 and the suite itself from 1,055.6 s to **962.8 s**. The median moved only 75.9% → 73.3%, because it is no longer set by declarations at all: **23 of 50 merges widen to the full suite before any `COVERS` is consulted**, through the `schema/`, `bin/lib/`, `viewer/parsers.py` and `tests/` helper rules. Reaching 50% would mean narrowing those, which trades real coverage inside an executor round for speed, and the user declined it: the widening rules stand as § 5.2 writes them. What phase A bought is the floor and the 93 seconds, not a halved median. `TASK-449` is unblocked.
- 2026-09-16 — **`affected` subtracts the slow tier, and says so on every run** — user decision `USER-944`, from `TASK-449`. § 5.1's table says `affected` is "modules selected by the change, plus `smoke`", and the implementation holds back any selected module in `tests/parallel § HARNESS_SELF_TESTS`, printing `held back — <module> is the slow tier's … run bash tests/run --tier slow` for each. The reason is arithmetic: `select()` draws from every module on disk, `full` excludes the slow set, so a widened selection would be a strict SUPERSET of `full` — an executor round would pay `test_tree_guard.py`'s 62.5 s to run the cheap tier. What it costs: a change that genuinely touches the harness selects those modules and is told to run `--tier slow` rather than running them, so the executor loop can miss a harness regression until the full run at merge. Three tests hold the rule, including one that a held-back module is named rather than silently dropped.
## 10. References
