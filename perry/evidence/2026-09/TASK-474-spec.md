...
1. `phase new <slug>` assigns the next unused phase number, zero-padded to three digits, computed from the phase directory's existing documents. In one locked operation it writes `phase/<NNN>-<slug>.md` from `goals/state/phase_TEMPLATE.md` with the supplied body, stamps `**Started**` to today and `**Status**` to `active`, and writes `phase/CURRENT`.

   **Corrected on 2026-09-20, before implementation.** This criterion first said `new` must *not* set a start date, write `phase/CURRENT` or make the phase active. That was read out of `goals/reference/phases.md § plan-phase`, whose sentence — "A draft does not reserve a number, set a start date, make a phase active or change `phase/CURRENT`" — governs the **chat draft** that precedes approval, not the writer that follows it. The same page, four paragraphs later, states the opposite about the writer: "The eventual writer's returned result must identify the ten-section phase prose **and the activated phase pointer**." Creation and activation are one operation. `activate` below remains a separate verb for re-pointing at a document that already exists.
...
4. `phase activate <NNN>` writes `phase/CURRENT` for a phase document that already exists. It, and `phase new`, are both refused, by name, while another phase is already active — the refusal names the active phase and does not close or replace it. `goals/reference/phases.md § plan-phase` requires a lifecycle choice here rather than a silent replacement.
