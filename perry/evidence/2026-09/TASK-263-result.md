...
| **Note on `stamp_last_updated`** | it rewrites `> Last updated:` in the rendered file by regex (`LAST_UPDATED_RE`). Its destination is the renderer: a header line emitted from the store's own write timestamp, not patched into text afterwards. Deletion dependency: the same wholesale re-render as the rest of this group. |
