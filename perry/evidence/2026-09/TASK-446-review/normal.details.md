...
Source: `compact.json`, `next.json`, captured with `bin/perry-state --compact` and `bin/perry-state --section next` in `/private/tmp/perry-scratch/Perry/phase004-k4_waa0n/review-task-446`; compact generated_at `2026-09-17T16:47:14`. State root is that checkout's `perry/`. This is a capture of the pinned checkout, not a claim about later main. Titles resolved by `bin/perry-explain`, retained in the corresponding ID.txt files. Active software-ops glossary maps Phase to Phase and Commitment to Key result; no vocabulary changes needed here.
...
Reviewer reuse: inspected fact-by-fact against fresh compact/next, same pinned PMO state. Detail wording retained from the author exhibit; semantic judgment is independently recorded in review.md.
