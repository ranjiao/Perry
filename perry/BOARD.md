...
| ID | Title | Owner | Status | Next action | Evidence | Verification | Depends on | Track | Stage | Arrived |
|---|---|---|---|---|---|---|---|---|---|---|
| TASK-040 | risks are still read from a markdown table and are not records in the store | Coding Agent | not_started | USER-016 answered — the claims[] declaration landed, and it was NOT sufficient: cmd_risks_write does not exist, the dispatch only printed a refusal. The refusal now reads schema/state-schema.json and names whichever gap is open. Remaining scope is the write path itself: risks-write --from-board, section -> store, the one-way import perry-tasks write --from-board is for tasks | evidence/2026-08/TASK-040-result.md | V3 | USER-016 | main |  |  |
