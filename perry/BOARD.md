...
| TASK-170 | an answered USER- ask is in no register a consumer can query | Coding Agent | in_progress | first dispatch 2026-08-21 18:07 was killed at 44s by an ESC on the main session — no work written, worktree clean at 923c129. Re-dispatched | — | V3 | — | main |  |  |  |  |  |  |
...
| TASK-178 | the read-only web viewer is deleted; aiMark is the consumption surface | Coding Agent | not_started | decided by the user 2026-08-21: aiMark exists, so the local console has no value. Measured boundary: NOTHING outside viewer/ imports serve.py — bin/perry-state:9 only MENTIONS it in a comment and the other greps are row.serves_kr matching as a substring. Flask, markdown and jinja2 are Perry's ONLY non-stdlib dependencies and they live entirely here, so this also makes the phase Cost Ceiling's claim true in fact and removes the two test modules that go red on any interpreter without jinja2 | — | V3 |  | main |  |  |  |  |  |  |
...
| TASK-176 | contract_key_parity refuses two identically shaped containers, so a payload can never grow a second list of an existing entry shape | Coding Agent | not_started | NEEDS THE ARGUMENT, not just the code: this changes how KR-O2.4 is MEASURED, and that KR's whole value is being a ruler nobody adjusted. TASK-040 hit it — adding cleared_items[] took KR-O2.4 from 0 to 22 because items[] and cleared_items[] are the same eleven keys — and correctly backed its change out rather than touch the instrument. The spec must say when one key table serving several containers is legitimate and when it is laziness | — | V4 |  | main |  |  |
