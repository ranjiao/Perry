...
**Not a version, 2026-08-28 (TASK-167).** `purge` is a new kind — the writer
gained `perry-task purge`, the store's only removal path — and § The event
kinds carries it because that list is derived from the writer, so the row
landed on the commit that added the subcommand rather than whenever somebody
next read the page. **No key was added, removed or retyped, and no existing
value changed meaning**, so the version does not move and `semantics` gains
nothing: this is a new *value* of `event`, which rule 2 does not govern and
which § The three rules already requires a consumer to survive — a kind you do
not recognise has always been possible on this feed and has always meant
"render it as history you have no special handling for".

**What a consumer that DOES branch should know about it.** `purge` is the only
kind that says a task stopped existing rather than stopped moving. A front-end
holding a cached row must drop it; a consumer deriving its own store from this
log must honour the removal or it will rebuild a record Perry deleted, which is
exactly what `bin/perry-task § _cmd_list_from_board` does with it. The removed
record rides on the LOG LINE under `record` and is not projected onto this
payload — § An event is the fifteen keys this feed carries, and a sixteenth for
one kind would be a key most events do not have.
