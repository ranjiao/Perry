...
| [`viewer/parsers.py`](../viewer/parsers.py) | **The only markdown parser.** `perry-task`, `perry-tasks`, `perry-state`, `perry-goals`, `perry-decide`, `perry-lint`, `perry-diagnose`, `perry-explain`, `perry-config`, `perry-knowledge`, `perry_store.py` and `perry_md_store.py` all import it. It is named for a web console that no longer exists (TASK-178); the directory is kept because renaming it touches 48 files' imports (measured 2026-09-14). |
...
And four more reach the **cadence register** — ADR-007 a fifth time,
`perry/cadence.jsonl` against `BOARD.md § Cadence` (TASK-237 deliverable 3b):

    perry-tasks cadence-build  [--root <p>]  derive the cadence store; write nothing
    perry-tasks cadence-render [--root <p>] [--write]  the store → `## Cadence`
    perry-tasks cadence-write  [--root <p>] --from-board  the section → the store
    perry-tasks cadence-diff   [--root <p>]  render and byte-compare with the file

`perry-task cadence-add` and `cadence-done` write this store with or without
`BOARD.md`. The import stores a cell as written: `Next due` carrying prose, and
frequencies such as `continuous` or `hourly` with no computable due date, are
live data on real registers, and `perry-state § cadence` reports them rather
