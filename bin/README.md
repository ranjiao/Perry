...
| [`perry-churn`](perry-churn) | read | Per-day line churn from `git log --numstat`, with documentation (`.md` and friends) counted apart from everything else. `--days N` prints the last N days as a calendar, so a day with no commits shows as a zero row rather than vanishing. Repository-agnostic: no Perry state is read, `-C <dir>` points it at any git repo, and `--csv` / `--json` carry the same numbers as the table. |
