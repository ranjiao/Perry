...
3. If pivoting the overall OKR → run `revise` to bump the version. If only the current phase → first run `/okr snapshot` to preserve the pre-pivot state, then write a `## Changes` line in `phase/<NNN>-<slug>.md` with `YYYY-MM-DD — <what> — <reason>`. Old text stays as strikethrough. **Each approved KR change — title, metric, drop, add — is one `perry-goals kr restate | withdraw | add` call with the same `--reason`** (`phases.md § kr`), never a hand edit of `linkage.jsonl` or `okr.jsonl`.
