...
If the project still has the **old monolithic `DECISIONS.md`** (one big file with all ADRs inline, no `decisions/` directory): PMO surfaces a one-time migration suggestion in the standup. Run `/pmo decide --migrate-from-monolithic` to split into `decisions/` and rebuild the index (irreversible — but git history preserves the original file). See `reference/decisions.md § Migration`.
