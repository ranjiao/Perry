...
- **Added**: 2026-05-01 (from ADR-007 / incident 2026-04-30-deploy-stuck / manual)
...
- **hard**: dispatch pre-flight refuses any task whose `Touches invariants:` includes a hard invariant unless the user explicitly waives. Used for things like "no production calls outside of `release/`", "no new top-level dependency without ADR", "no I/O bypassing layer Z".
