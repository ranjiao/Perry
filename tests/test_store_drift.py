...
**TASK-262 Amendment (4), round 4b retired the task store's half of this.**
A held `BOARD.md` is read by no check, so there is no file for the store to
drift from: `store_drift.comparison_performed` is `false` on every project
and the census says no board file is read. The tests that planted a hand
edit in a held board and asserted the finding, the cap, the order finding,
the derivation failure and the log gate's reappearing finding were deleted
with it (named in `perry/evidence/2026-09/TASK-262-round4b-result.md`). What
stays: no store is not a clean store, a badly-typed store is still named,
one check still may not kill the lint, `perry-state` still declines without
a log, and the OKR and config stores — which project from documents that
are not retired — keep every case.
