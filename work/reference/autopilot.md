...
3. Read the BOARD through the payload, not by eye:
   ...
   - **Skipped — high-stakes**: the row touches something on the project's high-stakes list → never auto-dispatched even if `Dispatch mode: auto` (safety > flag). **You make that judgement per row, by working `dispatch.md` pre-flight step 4 (4.1–4.6) against the spec.** There is no command that returns the verdict: `--escalation-scan` was removed on 2026-09-04 because Python must not judge a document's meaning (`USER-916`, `ADR-007` decision 3, `TASK-339`), and its last measurement was that formatting alone moved its verdict in both directions.
     **Autopilot runs unattended, so step 4.6's `ask` outcome resolves here as a skip, not as a question.** Anything other than a clean allow — a foreign-rooted path, a doing you cannot separate from a naming, an unarmed list, a spec offering neither `## Files in scope` nor `## Deliverable` — is skipped and named in the run report with the line that caused it. Interactive `/pmo dispatch` may ask the user; this loop may not, so it takes the safe direction instead.
