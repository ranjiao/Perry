...
4. **Safety re-validation**: `"$PERRY_HOME/bin/perry-state" --escalation-scan evidence/<YYYY-MM>/<TASK-ID>-spec.md`. **Exit code is the verdict — 0 proceed, 3 refuse, 4 unarmed (see below), 2 the spec path is wrong.** Do not perform this match by hand. The command scans `Files in scope` / `Deliverable` / `Out of scope` against the escalation union and applies both rules for you: a hit in `Files in scope` or `Deliverable` means the task touches it and refuses; a hit in `Out of scope` means the spec has stated in writing that it does not, and green-lights that fragment. The JSON names `refuse`, `green_lit` and `origins` so a refusal can be quoted back to the user with the line it came from.
