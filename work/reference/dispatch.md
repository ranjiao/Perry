...
4. **Safety re-validation**: `"$PERRY_HOME/bin/perry-state" --escalation-scan evidence/<YYYY-MM>/<TASK-ID>-spec.md`. **Exit code is the verdict — 0 proceed, 3 refuse, 4 unarmed (see below), 2 the spec path is wrong.** Do not perform this match by hand. The command scans `Files in scope` / `Deliverable` / `Out of scope` against the escalation union and applies the rules for you: a hit in `Files in scope` or `Deliverable` means the task touches it and refuses; a hit in `Out of scope` means the spec has stated in writing that it does not, and green-lights that fragment — **except where `Files in scope` declared it.** `Files in scope` is the enumerated list of paths the round will write, so a spec that lists a path there and also disclaims it has contradicted itself; the gate reports that in `contradictions` and refuses, rather than resolving it by dispatching. Three specs were passing on exactly that reading, over `schema/state-schema.json` (TASK-290).

   **A fragment is matched where it is written, not merely where it appears.** A match sitting strictly inside one component of a path names a longer file (`bin/perry-diagnose`, `reference/diagnose.md` — not the `diagnose` pipeline), and a bare directory fragment on a path rooted in *this* project names this project's own tree, which the hook's *"overwriting a project's **own** `design/`, `evidence/`, `knowledge/`, `inputs/`"* is not about. Both discounts are per-occurrence, one live occurrence outranks any number of discounted ones, and **every discount is listed in `discounted` with the path that caused it** — read that key before treating a `pass` as clean, because a discount nothing reports is a narrowing. `schema/state-schema.json` fills its own component and keeps refusing.

   The JSON names `refuse`, `green_lit`, `contradictions`, `discounted`, `origins` and `scope_scanned` so a refusal can be quoted back to the user with the line it came from — and so a `pass` can be told apart from a scan that had nothing to read.

   **The match is at word edges, not on bare substrings** (`viewer/parsers.py § escalation_pattern`, TASK-107). `origin` matches `git push origin main` and not "its original bytes"; `design/` still matches every path beneath it. Reading the union yourself and eyeballing substrings is what this step used to say, and it refused two dispatches in one day over "original" and "adopted" — a gate that cries wolf on ordinary English gets waved through, and rewording the spec became the cheapest way to pass it. `escalation.union` on `--section project` remains the list to *show* a user; it is not the thing to match by hand.
   ...
   **If the list is missing or empty** (exit 4, `verdict: unarmed`; equivalently `escalation.armed: false`, or `hook.high_stakes_armed: false` on a roleless project), this scan has nothing to match and would pass everything. Do not treat that as a pass. Tell the user in one line — *"no high-stakes list in `.perry/hook.md`, so nothing is being screened; `/pmo` bootstrap writes the default list"* — and require an explicit go-ahead in chat before dispatching. `AskUserQuestion` is not sufficient here; this is a safety gate, per `SKILL.md § User-prompt convention`. (`/pmo autopilot` refuses outright in the same situation — see `autopilot.md` pre-flight step 0.)

   **If `scope_scanned` is `[]`, the spec is the empty half — read it the same way.** `scope_scanned` names which of `Files in scope` / `Deliverable` the spec actually offered, non-empty, in the `## ` shape `_section` reads; `[]` means **neither**, so every fragment was matched against the empty string and this `pass` is a scan of nothing, not a clean one. **The exit code is still 0 and does not change** — 45 of Perry's own 135 specs are in that state, and a refusal here would retroactively stop every dispatch that touches them — so the exit code cannot be what tells you, and this key is. Do not treat it as a pass. Tell the user in one line — *"this spec offers `Files in scope` / `Deliverable` in a shape the gate cannot read, so nothing in it was screened; `subcommands.md § add-task` step 3 has the `## ` shape"* — and require an explicit go-ahead in chat before dispatching, exactly as for `unarmed` above; `AskUserQuestion` is not sufficient, same reason. The two halves are one rule: `armed` says whether the HOOK had anything to match with, `scope_scanned` says whether the SPEC had anything to match against, and an empty either side is a gate that passes everything. Only the hook half had a voice until TASK-284. `perry-lint --root .` reports the same condition across every spec at once, before any single dispatch reaches this step — it runs in the default pass, so nobody has to know to ask for it; `perry-lint --specs --json` is the same check with the full list rather than the first ten.
