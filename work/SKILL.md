...
4. **Read full text only when the current question needs it.** The payload already answers "how many / how stale / what's blocked". Open the actual file when the user asks about its content:
   ...
   The attribution rule still governs anything you roll up: a task's KR is resolved by stable ID through `linkage.jsonl` (`$PERRY_HOME/reference/okr-linkage.md`). `perry-state` applies exactly that rule — anything it reports as `unlinked` must be **asked about, never fuzzy-matched**.
...
On yes → read `reference/bootstrap.md` and follow the procedure (writes `.perry/config.jsonl` first with `perry-config set` when it is absent, creates initial state files, writes ADR-001, writes `.perry/hook.md` with the default high-stakes safety list and asks the user to confirm it, lazy-defers `ARCHITECTURE.md` / `runbook/` / `incidents/` until first use, then runs the standup).
...
- **Never guess a task's KR attribution — this is a hard gate.** Resolve every task/progress-report to a KR by stable ID through `linkage.jsonl` (explicit `kr:` → Project ID → registered alias; `$PERRY_HOME/reference/okr-linkage.md`). If it doesn't resolve to exactly one KR — because a name drifted, is ambiguous, or matches nothing — **ask the user** (`AskUserQuestion`, candidate KRs); if the user is unavailable, mark the task `attribution: unlinked`, keep it out of every KR roll-up, and surface it. Never fuzzy-match a name into a KR, and never fabricate a mapping to complete a number. New aliases confirmed by the user are handed to `okr` to record (PMO doesn't write `phase/`).
