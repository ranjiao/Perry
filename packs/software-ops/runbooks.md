...
These three are not narrative — they're machinery. "It will log on success" is not a success signal; `grep "DEPLOY_COMPLETE" logs/deployer.log | tail -1` is.
...
### {{Failure mode 1, e.g., "staging connection drops"}}
- **Symptom**: {{what the user sees}}
- **Diagnose**: `{{single command}}`
- **Canned fix**: `{{single command or named op (e.g., Slack /restart_deploy)}}`
...
Canned op convention: <e.g., Slack bot commands prefixed with /op_>
