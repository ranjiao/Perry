...
- A production / external-system action diverges from what was intended.
...
Naming: ISO date + 1–3 word slug. Examples: `2026-05-12-deploy-stuck.md`, `2026-05-13-alert-bot-silent.md`. Slug should match the affected runbook's slug when possible.
...
One paragraph. Why did this happen? Be specific — not "a bug in the code" but "the deploy-daemon's reconnect loop didn't reset its backoff counter on success, so after one transient failure it stayed at 5-minute reconnect intervals indefinitely."
...
- **Standup dashboard**: when `incidents/` exists, the dashboard adds a line: `🔥 Incidents · <open> open · <month> this month · <derived>/<total> w/ derived`. (Canonical format lives in `work/SKILL.md` standup dashboard — keep in sync.) Omit if directory missing.
